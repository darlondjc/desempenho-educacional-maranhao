...
\begin{itemize}
  ...
  \item \textbf{IBGE:} remoção de linhas de totalização regional, normalização do nome do município para maiúsculas sem acentos (para compatibilidade com o join).
...
Os percentuais médios das variáveis de infraestrutura revelam disparidades: a conectividade à internet está presente em 73,7\% das escolas municipais do cluster vulnerável e em 81,9\% das escolas do cluster de maior renda; já o acesso a biblioteca ou sala de leitura é notavelmente baixo em ambos os grupos (12,5\% nas escolas vulneráveis), sugerindo que esse recurso pode ser um diferencial relevante justamente por sua raridade. A proporção média de docentes EFAI com ensino superior é de 60,5\% nos municípios mais vulneráveis e de 73,9\% naqueles de maior renda, diferença que antecipa uma das hipóteses testadas no estágio intragrupo.

\section{Modelagem}

\subsection{Seleção do Número de Clusters}

A Figura~\ref{fig:elbow} apresenta as curvas de inércia (cotovelo) e coeficiente de silhouette para $k \in \{2, \ldots, 10\}$. O coeficiente de silhouette atinge seu valor máximo em $k = 2$ (0,619), indicando dois grupos bem separados no espaço {renda, IDHM}. O método do cotovelo confirma a escolha: a queda marginal de inércia entre $k = 2$ e $k = 3$ é consideravelmente menor que entre $k = 1$ e $k = 2$. A estabilidade bootstrap (ARI médio = 0,89 em 25 iterações) confirma que a partição é robusta.

\begin{figure}[H]
  \centering
  \includegraphics[width=0.90\textwidth]{elbow_silhouette.png}
  \caption{Curvas de inércia (esquerda) e coeficiente de silhouette (direita) para $k \in \{2, \ldots, 10\}$. A linha vertical vermelha indica o $k$ ótimo segundo o silhouette.}
  \label{fig:elbow}
\end{figure}
...
A Figura~\ref{fig:rf_vulneraveis} apresenta as importâncias do Random Forest Classifier para o cluster vulnerável. As oito variáveis de maior importância são, em ordem decrescente: renda per capita (0,121), média de alunos por turma no EF básico (0,098), \% escolas com biblioteca/sala de leitura (0,096), \% docentes com ensino superior (0,093), proxy de distorção idade-série (0,090), \% docentes com licenciatura (0,087), média de alunos por turma no EF anos finais (0,074) e \% escolas com esgoto adequado (0,073).

\begin{figure}[H]
  \centering
  \includegraphics[width=0.85\textwidth]{cluster_vulneraveis_variaveis_relevantes_destaque.png}
  \caption{Importâncias do Random Forest Classifier para diferenciar municípios de alto e baixo IDEB no cluster vulnerável. As barras representam a redução média de impureza de Gini.}
...
\midrule
\% doc. com licenciatura     & 68,0\% & 59,4\% & +8,6 p.p. & 0,023** \\
\% doc. com ensino superior  & 66,8\% & 58,5\% & +8,3 p.p. & 0,026** \\
\% escolas com biblioteca    & 13,8\% & 10,5\% & +3,3 p.p. & 0,030** \\
\% escolas com internet      & 77,1\% & 69,5\% & +7,6 p.p. & 0,062*  \\
Alunos/turma EF (média)      & 22,9   & 21,9   & +1,0      & 0,066*  \\
\bottomrule
\multicolumn{5}{l}{\footnotesize Sig.: ** $p < 0{,}05$; * $p < 0{,}10$.}
\end{tabular}
\end{table}

A formação docente emerge como o fator intracontextual mais consistentemente associado ao desempenho superior: municípios vulneráveis de alto IDEB têm, em média, 8,5 p.p. a mais de docentes com ensino superior e 8,6 p.p. a mais com licenciatura específica em comparação aos de baixo IDEB. Esse achado é convergente com a literatura internacional. \citet{rivkin2005} demonstraram que a qualidade do professor é o fator intraescolar de maior impacto sobre o aprendizado. Estudos nacionais também apontam formação docente como variável central nos resultados do IDEB \cite{barbosa2021}.

O acesso a biblioteca ou sala de leitura, ainda que presente em apenas 10--14\% das escolas, também aparece como fator discriminante ($p = 0{,}030$). Sua raridade torna essa variável um alvo direto de intervenção: pequenas expansões na proporção de escolas com espaços de leitura estruturados podem fazer diferença mensurável no IDEB municipal.

A conectividade à internet ($p = 0{,}062$) e o tamanho médio de turma ($p = 0{,}066$) são significativos ao nível de 10\%, sinalizando tendências relevantes para políticas, embora com menor robustez estatística dada a variabilidade intragrupo.

A renda per capita aparece com a maior importância no Random Forest (0,121) mesmo dentro do cluster vulnerável, onde a variação de renda é estruturalmente menor. Isso sugere que, mesmo após a segmentação socioeconômica, a heterogeneidade residual de renda dentro do grupo tem poder preditivo, o que reforça a importância de controles socioeconômicos em análises educacionais, mesmo dentro de grupos nominalmente homogêneos.
...
\textbf{Formação e habilitação docente.} A diferença de 8,5 p.p. na proporção de docentes com ensino superior entre municípios de alto e baixo IDEB, dentro do mesmo contexto socioeconômico, indica que programas de formação continuada e de indução à licenciatura específica têm potencial de impacto real sobre o IDEB. Políticas estaduais e federais de financiamento de graduação a distância para docentes em exercício (como o Parfor) são alvos naturais de expansão nos municípios do cluster vulnerável.

\textbf{Espaços de leitura nas escolas.} A escassez de bibliotecas e salas de leitura (presentes em apenas 10--14\% das escolas) e sua associação significativa com desempenho superior sugerem que investimentos relativamente modestos em acervo e espaço físico podem ser custo-efetivos. Municípios com IDEB baixo dentro do cluster vulnerável têm, em média, 3,3 p.p. menos escolas com esse recurso.

\textbf{Conectividade digital.} A diferença de 7,6 p.p. na proporção de escolas com internet aponta para o efeito mediado da conectividade sobre o IDEB: embora o impacto seja apenas marginalmente significativo ($p = 0{,}062$), a convergência com a importância no Random Forest justifica o monitoramento desse indicador como parte de estratégias de melhoria educacional.