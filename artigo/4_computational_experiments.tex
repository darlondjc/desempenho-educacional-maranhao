...
Após o merge, não há valores ausentes nas variáveis de cluster nem nas variáveis-alvo (IDEB), mas algumas variáveis explicativas apresentam missings pontuais decorrentes de municípios sem turmas de correção de fluxo (alunos com atraso no que se espera de idade/série).
...
A clusterização K-Means foi aplicada sobre as variáveis \textit{renda per capita} e \textit{IDHM} (normalizadas via RobustScaler), testando $k \in \{2, \ldots, 10\}$ conforme detalhado na Seção~\ref{cap:methods}.

\subsection{Seleção do Número de Clusters}

A Figura~\ref{fig:elbow} apresenta as curvas de inércia (cotovelo) e Silhouette Score para $k \in \{2, \ldots, 10\}$. O Silhouette Score atinge seu valor máximo em $k = 2$ (0,619), indicando dois grupos bem separados na relação \textit{renda/IDHM}. O Método do Cotovelo confirma a escolha: a queda marginal de inércia entre $k = 2$ e $k = 3$ é consideravelmente menor que entre $k = 1$ e $k = 2$. A estabilidade bootstrap (ARI médio = 0,89 em 25 iterações) confirma que a partição é robusta.
...
\begin{itemize}
  \item \textbf{Cluster vulnerável} (n = 197): renda per capita média de R\$ 210, IDHM médio de 0,737, IDEB médio de 4,96.
...
Dentro do cluster vulnerável (n = 197), os 57 municípios que se encontram no quartil superior de IDEB (alto, $\geq 5{,}20$, média 5,65) são comparados aos 50 do quartil inferior (baixo, $\leq 4{,}50$, média 4,35). A diferença média de 1,30 ponto de IDEB entre os grupos é substancial: equivale a mais de dois desvios-padrão da distribuição estadual.
